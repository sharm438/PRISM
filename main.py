...
def parse_args():
    ...
    parser.add_argument("--cmax", help="PRISM's notion of c_max", default=2, type=int)
...
def main(args):
    ...
    for epoch in range(num_epochs):
        grad_list = []
        if (args.aggregation == 'prism'):
            susp = susp*decay #suspicion score only used by PRISM
        ...
        if (args.aggregation == 'fedsgd'):
            ...
            net, direction, susp, flip_new, weight = aggregation.prism(device, byz, lr, grad_list, net, direction, susp, flip_new, args.cmax, weight)
        ...
        with torch.no_grad():
            ...
            torch.save(net.state_dict(), filename+'_'+str(epoch)+'_model.pth')
